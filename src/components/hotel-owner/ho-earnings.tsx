...
import { Download, Banknote, Calendar, TrendingUp, Wallet, FileText, CreditCard, Plus, BarChart3 } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard icon={TrendingUp} label={t("this_month", lang)} value={formatCurrency(thisMonth, lang)} color="accent" trend="+22%" trendUp delay={0.05} />
        ...
        <Card className="border-border/70">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-muted-foreground">{lang === "ar" ? "متوسط السعر اليومي (ADR)" : "Avg Daily Rate (ADR)"}</span>
            </div>
            <p className="text-xl font-bold">{formatCurrency(1240, lang)}</p>
          </CardContent>
        </Card>
        <Card className="border-border/70">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              <BarChart3 className="h-4 w-4 text-primary" />
              <span className="text-xs text-muted-foreground">{lang === "ar" ? "الإيراد لكل غرفة (RevPAR)" : "RevPAR"}</span>
            </div>
            <p className="text-xl font-bold">{formatCurrency(892, lang)}</p>
          </CardContent>
        </Card>
        <Card className="border-border/70">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              <TrendingUp className="h-4 w-4 text-[oklch(0.5_0.15_145)]" />
              <span className="text-xs text-muted-foreground">{lang === "ar" ? "إيراد الغرف" : "Room Revenue"}</span>
            </div>
            <p className="text-xl font-bold">{formatCurrency(245000, lang)}</p>
          </CardContent>
        </Card>
        <Card className="border-border/70">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              <TrendingUp className="h-4 w-4 text-[oklch(0.5_0.13_30)]" />
              <span className="text-xs text-muted-foreground">{lang === "ar" ? "خدمات إضافية" : "Extra Services"}</span>
            </div>
            <p className="text-xl font-bold">{formatCurrency(38400, lang)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-border/70">
        <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-base">{lang === "ar" ? "اتجاه الإيرادات" : "Revenue Trend"}</CardTitle>
